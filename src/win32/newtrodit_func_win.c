...
void LoadLineCount(File_info *tstack, int startpos, int starty)
{
    if (lineCount)
    {
        ...
        {
            if (k + skipamount < tstack->bufy && tstack->strsave[k + skipamount][0] != '\0' && k + skipamount < startpos + (n - 1) && k + skipamount < tstack->linecount) // Try to skip 'skipamount' lines for optimization
            {
...
int WriteBuffer(FILE *fstream, File_info *tstack)
{
    if (tstack->utf8)
    {
        ...
        {
            if ((sz = strlen_n(tstack->strsave[i])) < BUFFER_X * sizeof(wchar_t))
            {
...
int DisplayFileContent(File_info *tstack, FILE *fstream, int starty)
{
    ...
    {
        if (i < tstack->bufy - 1 && tstack->strsave[i][0] != '\0')
        {
...
int SaveFile(File_info *tstack, char *fname, bool save_dialog)
{
    ...
    if (WriteBuffer(fp, tstack)) // Write the file content
    {
        ...
        memset(tstack->filename, 0, MAX_PATH); // Empty the old filename
        memcpy(tstack->filename, tmp_filename, strlen_n(tmp_filename));
        if (tstack->is_untitled || !tstack->is_saved)
        {
...
int InsertNewRow(File_info *tstack, int *xps, int *yps, int dispy, int size, bool refreshScreen, bool addNewline)
{
...
int ReturnFindIndex(int insensitive, char *str, char *find)
{
    ...
    {
        char *s_d = strdup(str);
        char *s_f = strdup(find);
        find_index = FindString(strlwr(s_d), strlwr(s_f));
        free(s_d);
        free(s_f);
    }
    else
    {
        find_index = FindString(str, find);
    }
    return find_index;
}
        ...
        if (irInBuffer.EventType == WINDOW_BUFFER_SIZE_EVENT)
        {
            if (allowAutomaticResizing)
            {
                ...
                if ((oldSize.X != newSize.X || oldSize.Y != newSize.Y))
                {
                    ...
                    Sleep(1); // This unnoticeable sleep is to avoid graphical bugs
                    clearAllBuffer = true;
                    LoadAllNewtrodit();
                    clearAllBuffer = false;
                    DisplayFileContent(tstack, stdout, 0);
                    DisplayCursorPos(tstack);
                    DisplayLineCount(tstack, YSIZE - 3, tstack->display_y);
...
{
    char *fileopenread = calloc(MAX_PATH + 1, sizeof(char));
    int n = 0;
    if (tstack->is_modified)
    {
        PrintBottomString("%s", NEWTRODIT_PROMPT_SAVE_MODIFIED_FILE);
    ...
    RemoveQuotes(fileopenread);

    for (int i = 0; i < open_files; ++i) // Check if file is already open in another tab
    {
        if (!strcmp(StrLastTok(Tab_stack[i].filename, PATHTOKENS), fileopenread) && i != file_index)
        {
            PrintBottomString(NEWTRODIT_PROMPT_ALREADY_OPEN_TAB, fileopenread);
            if (YesNoPrompt())
            {
                file_index = i;
                LoadAllNewtrodit();
...
char *FindNewtroditString(File_info *tstack, char *find_string)
{
    /* size_t find_index = 0, linelen=0, findlen = strlen_n(find_string);

    bool findInsensitive = false;
    for(size_t i = 1; i < tstack->linecount; i++)
    {
        linelen = strlen_n(tstack->strsave[i]);

        while((find_index = ReturnFindIndex(findInsensitive, tstack->strsave[i] + find_index, find_string)) != -1)
        {
            PrintBottomString("Found %s at X:%d", find_index);
            getch_n();
        }
        find_index = 0;
    }
    return NULL; */
    size_t findlen = strlen_n(find_string);
    char wholeWordDelims[] = "`~!@#$%^&*()[]{}-=+\\|/;:\'\",.<>? \n\t\r";
    bool found = 0;
    size_t index_total = -1, pos = 0;
    for (int i = 1; i < tstack->linecount; i++)
    {
        for (size_t k = 0; k <= strlen_n(tstack->strsave[i]); k++)
        {
            if (tstack->strsave[i][0] != '\0')
            {
                index_total = 0;
                while ((pos = ReturnFindIndex(findInsensitive, tstack->strsave[i] + index_total, find_string)) != -1)
                {
                    found = true;
                    index_total += pos;
                    printf("Found at line %d at k=%u\n", i, index_total);
                }
            }
        }
        char *occurrence = tstack->strsave[i];
        int offset = 0;
    }
    getch_n();
    return 0;
...
/*
while ((occurrence = strstr(occurrence + offset, find_string)) != NULL)
        {
            offset = occurrence - tstack->strsave[i];
            PrintBottomString("Found at line %d, position %d: %s", i, offset + 1);
            getch_n();
            found = 1;
            offset++;
        }
*/
char *FindXNewtroditString(File_info *tstack, char *find_string)
{
    ...
    for (size_t i = 0; i <= Tab_stack[file_index].linecount; i++)
    {
        ...
        while (n >= 0 && c != 27)
        {
            ...
            find_index = ReturnFindIndex(findInsensitive, Tab_stack[file_index].strsave[i] + find_index, find_string);
            if (find_index >= 0)
            {

                if (!matchWholeWord || (matchWholeWord && IsWholeWord(Tab_stack[file_index].strsave[i] + find_index + n, find_string, wholeWordDelims)) > 0)
                {
                    ...
                    SetCharColor(strlen_n(find_string), FIND_HIGHLIGHT_COLOR, find_index + n + (lineCount ? Tab_stack[file_index].linecount_wide : 0), Tab_stack[file_index].display_y);
                    SetColor(bg_color);
                    ShowFindMenu();
                    gotoxy(find_index + n + (lineCount ? Tab_stack[file_index].linecount_wide : 0) + strlen_n(find_string), Tab_stack[file_index].display_y);
                    ...
                    if (!(c & BIT_ESC224))
                    {
                        ...
                        else if (c & BIT_ESC0 && (c & ~BIT_ESC0) == F4) // F4 = Toggle case sensitive
                        {
                            ...
                            gotoxy(find_index + n + (lineCount ? Tab_stack[file_index].linecount_wide : 0) + strlen_n(find_string), Tab_stack[file_index].display_y); // Keep the cursor in the same place
                            getch_n();
                            c = -1;
                        }
        ...
        _xpos = find_index + strlen_n(find_string);
        if (c == ESC)
        {
            break;
        }
...
char *PasteClipboardNewtrodit(File_info *tstack)
{
    char *ptr, *buffer_clipboard;
    size_t n = 0;

    char *buf_cleared;

    buffer_clipboard = GetClipboardNewtrodit(&n, false);
    FILE *fp = fopen("outclip.txt", "wb");
    fputs(buffer_clipboard, fp);
    fclose(fp);
    char *spacebuf = PrintTab(TAB_WIDE);
    if (buffer_clipboard != NULL)
    {
        if (convertTabtoSpaces)
        {
            buf_cleared = calloc(TAB_WIDE * n + 1, sizeof(char));
            ...
            for (size_t i = 0; i < n; i++)
            {
                ...
                {
                    printf("'%s'", TAB_WIDE);
                    strncat(buf_cleared, spacebuf, TAB_WIDE);
                    cnt++;
                }
                else
                {
                    buf_cleared[i + cnt * TAB_WIDE] = buffer_clipboard[i];
                }
            }
        }
        else
        {
            buf_cleared = buffer_clipboard;
        }

        ptr = strtok(buf_cleared, "\n");
        do
        {
            ptr[strcspn(ptr, "\r\n")] = '\0';
            n = tstack->bufx - strlen_n(tstack->strsave[_ypos]);
            // strncpy_n(tstack->strsave[_ypos], ptr, tstack->bufx);

            if (strlen_n(ptr) < n)
            {
                if (LineContainsNewLine(tstack, _ypos))
                {
                    memset(tstack->strsave[_ypos] + strlen_n(tstack->strsave[_ypos]) - strlen_n(tstack->newline), 0, strlen_n(tstack->newline)); // Remove newline if it is present
                }
                InsertStr(tstack->strsave[_ypos], ptr, _xpos, false, tstack->bufx);
            }
            ptr = strtok(NULL, "\n");
            if (!(tstack->linecount < tstack->bufy - 1 && _ypos < tstack->bufy - 1))
            {
                break;
            }
            if (ptr)
            {
                ...
                {
                    if (InsertNewRow(tstack, &_xpos, &_ypos, tstack->display_y, tstack->bufx, false, false))
                    {
                        PrintBottomString("InsertNewRow OK");
                        getch_n();
                    }
                }
                if (!LineContainsNewLine(tstack, _ypos))
                {
                    strncat(tstack->strsave[_ypos], tstack->newline, strlen_n(tstack->newline));
                }
                tstack->linecount++; // Increase Y position and linecount, and set X position back to zero
                _ypos++;
                _xpos = 0;
            }
        ...
        DisplayFileContent(tstack, stdout, 0);
        _xpos = NoLfLen(tstack->strsave[_ypos]);
    }
    else
    {
        PrintBottomString("%s", NEWTRODIT_ERROR_CLIPBOARD_PASTE);
        WriteLogFile("%s", NEWTRODIT_ERROR_CLIPBOARD_PASTE);
        getch_n();
        ShowBottomMenu();
        DisplayCursorPos(tstack);
    }
